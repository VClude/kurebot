...
const spec_config = require('../bot.config.json');
const bot_config = require('../spec.config.json');
const prefix = spec_config.bot_config.prefix;
...
let pull = function(rateup,rateupa,rateups) {
    const weapSR = require(rateup);
    const weapA = require(rateupa);
    const weapS = require(rateups);
    let result = roll();
...
let pullSpec = function(rateup,rateups,isRateup) {
    const weapS = require(rateups);
    const weapSR = require(rateup);
    let result = roll();
...
module.exports = {
        ...
        if(args.length === 0) {
            Object.keys(bot_config.event).forEach((key, index) => {
                ...
                const embed = new Discord.MessageEmbed()
                            ...
                            .setDescription('**!s spec ' + bot_config.event[key].args + '** to pull this banner')
    ...
    if(!bot_config.event[args[0]]) {
        const embed = new Discord.MessageEmbed()
                            ...
                            .setDescription('**!s spec** for available banner list')
                            .setColor(12745742)
                        message.channel.send({embed});
        return;
    }
    ...
    mys.doQuery(query,parser,function(results){
        ...
        if(res[0]){
            ...
            if(oldGem < tccostinit && isNotFree){
                ...
                let poolRateA = bot_config.event[args[0]].rateupA;
                let poolRateS = bot_config.event[args[0]].rateupS;
                    ...
                    mys.doQuery(query,parser,function(results){ 
                        ...
                        for(let i = 0; i < quantitygacha; i++){
                            ...
                            if(quantitygacha == 5 && i == 4 || quantitygacha == 11 && i == 10){
                                ...
                                a = pull(poolRate,poolRateA,poolRateS);
                                if(a.rarity == 'SR'){
                                    textE.push(util.evalRarity(a.rarity,client) + a.name);
                                    isSR = isSR + 1;
                                    srcontent.push({'weapid':a.resourceName,'weapname': a.name});
